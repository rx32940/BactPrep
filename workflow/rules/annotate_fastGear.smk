rule annotate_core_SNPs_alignment:
    ...
    output:
        os.path.join(fastGear_core_dir , str(project_prefix + "_core_mask_snp_meta.fasta"))
    params:
...
rule annotate_coreSNP_tree:
    input:
        ...
        metadata_file = metadata_file
    output:
        os.path.join(fastGear_core_dir , "fastgear_iqtree" , str(project_prefix + "_meta.coreSNPs.newick"))
    params:
        meta_include = metadata_include,
        key_column_index = biosample_column -1
    shell:
